...
def responseHandler(response):
    ...
    if response.status_code in conf.response_status_code:
        ...
        res={'code':str(response.status_code),'url':response.url,'size':str(size),'content-type':response.headers.get('content-type'),'content':response.text,'cookies':response.cookies,'time':str(response.elapsed.total_seconds())+'s'}
